...
def query_ByPeliculaAndFormato(peliculaId,fomatoId):
    funcion = session.query(Funcion).filter(Funcion.cancelada == None).filter(Funcion.id_pelicula == peliculaId).filter(Funcion.id_formato == fomatoId).filter(Funcion.fechaInicio == datetime.now().date() and Funcion.horaInicio > datetime.now().time() or Funcion.fechaInicio > datetime.now().date()).all()
    return funcion
